    ...
    void PatternDetector::initPatternDetector(const double param1, const double param2, const int param3, const double param4, const int param5, const int thresh_mode)
{
    ...
    block_size = param3;//for adaptive image thresholding - Size of a pixel neighborhood that is used to calculate a threshold value for the pixel: 3, 5, 7, and so on.
...
void PatternDetector::convertAndBinarize(const Mat& src, Mat& dst1, Mat& dst2, int thresh_mode)
{
	...
	if (thresh_mode == 1){
        threshold(dst2, dst1, thresh1, 255, THRESH_BINARY_INV);
	}
	else {
        adaptiveThreshold( dst2, dst1, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY_INV, block_size, thresh2);
	}
